    ...
    <section className="py-40 bg-white relative z-10">
      <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-20 items-start">
        <div className="sticky top-40">
          ...
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight text-slate-900 leading-[0.9] uppercase mb-8">
            Những thắc mắc <br />
            <span className="text-slate-400">thường gặp.</span>
          </h2>
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="bg-white border border-slate-100 rounded-3xl px-6 md:px-8 overflow-hidden shadow-sm"
              >
                <AccordionTrigger className="text-left py-6 hover:no-underline hover:text-orange-500 transition-colors font-bold text-slate-800 text-lg md:text-xl tracking-tight">
                  {faq.question}
                </AccordionTrigger>
