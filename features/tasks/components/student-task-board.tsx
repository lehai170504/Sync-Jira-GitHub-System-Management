...
import type { Sprint, Task } from "@/features/tasks/types/tasks-types";
import { isDateOverdue, isTaskOverdue, mapApiTaskToStatus, mapStatusToStatusName, formatDateVN_ddMMyyyy, formatDateVN_yyyyMMdd } from "@/features/tasks/utils/utils";
import { TaskBoardHeader } from "@/features/tasks/components/task-board-header";
import { MemberTaskBoardView } from "@/features/tasks/components/member-task-board-view";
...
export function TaskBoard() {
    const { startTour } = useStudentTour();
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="space-y-1">
                    <div className="flex items-center gap-3">
                        <h2 id="student-board-header" className="text-3xl font-bold tracking-tight flex items-center gap-2">
                            <LayoutDashboard className="h-7 w-7 text-[#F27124]" />
                            Team Task Board
                        </h2>
                        <Button variant="outline" size="icon" onClick={startTour} className="rounded-full border-slate-200 dark:border-slate-800 text-slate-500 hover:text-[#F27124] hover:border-[#F27124] transition-colors shrink-0 h-8 w-8"><HelpCircle className="h-4 w-4" /></Button>
                    </div>
                    ...
                    >
                        {isTeamAllTasksLoading ? (
                            ...
                            <MemberTaskBoardView
                                ...
                                selectedMemberId={selectedTableMemberId}
                                onMemberChange={setSelectedTableMemberId}
                                onViewTask={setViewTask}
                            />
                        )}
                    </TabsContent>
